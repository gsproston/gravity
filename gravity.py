...
class Asteroid:
    lineSpeed = 1 #change to 1 for no orbits
...
def draw(): #called when screen needs to be updated
    ...
    for ast in asteroids:
        pygame.draw.circle(screen,black,(int(ast.x),int(ast.y)),ast.r,1)
        pygame.draw.circle(screen,white,(int(ast.x),int(ast.y)),ast.r-1,0)
    for planet in planets:
        pygame.draw.circle(screen,black,(int(planet.x),int(planet.y)),planet.r,1)
...
if __name__ == "__main__":
    ...
    while (not shutdown):        
        ...
        while i < len(asteroids):
            ...
            crashed = False
            for planet in planets:
                d = math.sqrt(pow(asti.x-planet.x,2)+pow(asti.y-planet.y,2))
                if d < asti.r+planet.r-3: #collision, remove from queue
                    for point in asti.orbitalPath:
                    ...
                    break
            j = 0
            while j < len(asteroids) and not crashed:
                astj = asteroids[j]
                if i == j:
                    j += 1
                    continue
                d = math.sqrt(pow(asti.x-astj.x,2)+pow(asti.y-astj.y,2))
                if d < asti.r+astj.r-3: #collision, remove both from queue
                    for point in asti.orbitalPath:
                        pscreen.set_at(ipoint(point),background)
                    for point in astj.orbitalPath:
                        pscreen.set_at(ipoint(point),background)
                    #do they ricochet or merge?
                    #find momentums
                    mxi = asti.m*asti.vx
                    mxj = astj.m*astj.vx
                    myi = asti.m*asti.vy
                    myj = astj.m*astj.vy
                    if mxi*mxj > 0 or myi*myj > 0: #same direction
                        tol = 1
                        print("%f,%f,%f,%f" % (mxi,mxj,myi,myj))
                    else: #hit head on
                        tol = 0.5
                        print("%f,%f,%f,%f" % (mxi,mxj,myi,myj))
                    asteroids.pop(i)
